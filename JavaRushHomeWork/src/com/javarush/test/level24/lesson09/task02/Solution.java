...
public class Solution
{
    ...
    Collections.sort(list, new Comparator<Stock>()
    {
        public int compare(Stock stock1, Stock stock2)
        {
            ...
            if (!stock1.get("name").toString().equals(stock2.get("name").toString()))

                finalValuE = stock1.get("name").toString().compareTo(stock2.get("name").toString());
            else
            {
                if (((Date) stock2.get("date")).compareTo((Date) stock1.get("date")) != 0)

                    finalValuE = ((Date) stock2.get("date")).compareTo((Date) stock1.get("date"));
                else
                {
                    double finalChange1 = 0.0;
                    double finalChange2 = 0.0;

                    if (!stock1.containsKey("change"))
                        finalChange1 = (double) stock1.get("last") - (double) stock1.get("open");

                    else
                        finalChange1 = (double) stock1.get("change");

                    if (!stock2.containsKey("change"))
                        finalChange2 = (double) stock2.get("last") - (double) stock2.get("open");
                    else
                        finalChange2 = (double) stock2.get("change");

                    finalValuE = Double.compare(finalChange2, finalChange1);
                }
            }
            return finalValuE;
        }
